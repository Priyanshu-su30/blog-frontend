async function fetchUserSpecificBlogs(toast, userid, token, setBlogData) {
  const apiURL = `https://restaurant-kitchen-7tse.onrender.com/blog/userSpecificBlogs?id=${userid}`;
...
//   userId
// ) {
//   const apiURL = "https://restaurant-kitchen-7tse.onrender.com/blog/newBlog";
...
async function writeNewBlog(toast, token, h1, h2, h3, p1, p2, p3, blogImage, userId) {
  const apiURL = "https://restaurant-kitchen-7tse.onrender.com/blog/newBlog";
  const blogInformation = new FormData();
  const currentDate = new Date();
  ...
  try {
    ...
    toast("An error occurred while posting your blog.");
  }
}
...
async function updateBlog(
  ...
  const apiURL = `https://restaurant-kitchen-7tse.onrender.com/blog/updateBlog?id=${blogId}`;
...
async function deleteBlog(blogId, toast, token, userid, setBlogData) {
  const apiURL = `https://restaurant-kitchen-7tse.onrender.com/blog/deleteBlog?id=${blogId}`;
